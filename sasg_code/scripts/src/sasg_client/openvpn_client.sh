...
#    2018.05.08 - SBraun - v3.2.03 - added remote-cert-tls server to client side openvpn config for 
...
do
  if [[ -f ${FN}/client.conf.${SASG_IP_1[$INDEX]} ]];then
    echo "removing old client.conf.${SASG_IP_1[$INDEX]} file................."
    rm $FN/client.conf.${SASG_IP_1[$INDEX]}
  fi
  ...
  echo "Generating client.conf.${SASG_IP_1[$INDEX]}"
  # Create client.c nf Configuration Section
  echo "# Generated by $SCRIPT_INFO on $TIMESTAMP" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  ############################OpenVPN_client.conf###############################
  #echo "status /etc/openvpn/status/vtun0.status 30" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  echo "status ${SAS_CONFIG}/status/vtun0.status 30" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  echo "dev-type tun" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  echo "dev vtun0" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  echo "ping 10" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  echo "ping-restart 60" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  #echo "ifconfig 3.0.0.2 3.0.0.1" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  echo "ifconfig 3.0.0.$((INDEX+2)) 3.0.0.1" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  # if vars contains vyos side listener port variable, use that, otherwise use default of 1194
  if [ -z "${VTUN_PORT[$INDEX]}" ];then
    echo "rport ${VTUN_PORT[0]}" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  else
    echo "rport ${VTUN_PORT[$INDEX]}" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  fi
  echo "proto tcp-client" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  if [ $INDEX -eq 0 ];then
    echo "remote $PUBLIC_VYOS_IP" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  else
    if [ -z ${PORTABLE_VTUN_CIDR[$INDEX]} ];then
      ...
      echo "remote 9.9.9.9" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
    else
      echo "remote ${PORTABLE_VTUN_CIDR[$INDEX]%/*}" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
    fi
  fi
  echo "cipher aes-256-cbc" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  echo "tls-version-min 1.0" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  echo "remote-cert-tls server" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
...
#  echo "ca /etc/openvpn/auth/ca.crt" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
#  echo "cert /etc/openvpn/auth/$SASGCRT" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
#  echo "key /etc/openvpn/auth/$SASGKEY" >> $FN/client.conf.${SASG_IP_1[$INDEX]}

  echo "ca ${SAS_CONFIG}/auth/ca.crt" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  echo "cert ${SAS_CONFIG}/auth/$SASGCRT" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  echo "key ${SAS_CONFIG}/auth/$SASGKEY" >> $FN/client.conf.${SASG_IP_1[$INDEX]}

  echo "tls-client" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  echo "verb 3" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  # if an http proxy is defined, add entry in client.conf
  if [ ! -z ${HTTPS_PROXY} ];then
    echo "http-proxy ${HTTPS_PROXY} ${HTTPS_PROXY_PORT}" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
    echo "http-proxy-retry" >> $FN/client.conf.${SASG_IP_1[$INDEX]}
  fi
  echo "route 2.0.0.0 255.0.0.0" >> $FN/client.conf.${SASG_IP_1[$INDEX]}

  if [ ! -z ${SASG_IP_2[$INDEX]} ];then
    echo "Generating client.conf.${SASG_IP_2[$INDEX]}"
    cp $FN/client.conf.${SASG_IP_1[$INDEX]} $FN/client.conf.${SASG_IP_2[$INDEX]}
  fi
done
