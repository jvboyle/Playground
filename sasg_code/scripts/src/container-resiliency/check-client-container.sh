...
function show_usage {
  echo "
  RESILIENCY WATCHDOG FOR SGHA CONTAINERS
  
  FLAGS:
  * -h/--help - this message

  Script is intended to be run by cron and restart SGHA containers if they are 
  ...
  The client will NOT be checked if:
    * It is disabled (systemct disable sgha-resilient@clientA.service) or just
      not enabled
    * It is stopped manually (systemct stop sgha-resilient@clientA.service)
  ...
  systemct restart sgha-resilient@clientA.service
...
LOG_NAME="resiliency-watchdog.log"
...
function check_client {
  CLIENT="${1}"
  CLIENT_SERVICE_STATUS="$(systemctl is-active "sgha-resilient@${CLIENT}.service")"
  ...
  # Testing if we need to check at all
    ...
    if [ "$(systemctl is-enabled "sgha-resilient@${CLIENT}.service")" == "disabled" ]; then
...
for client in ${clients}; do
  ...
  case "${RC}" in
    ...
    $RC_CLIENT_DOWN)
      ...
      systemctl restart "sgha-resilient@${client}.service"
      ;;
