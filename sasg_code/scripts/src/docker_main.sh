...
CLIENT_ENV="${CLIENTS_DIR}/${DOCKER_SASFW}/sasg.env"
CLIENT_ENV2="${CLIENTS_DIR}/${DOCKER_SASFW}/.env"
SASG_BIN_DIR="/sasg/bin"
RHEL_RELEASE="$(cut -d: -f5 /etc/system-release-cpe | cut -d. -f1)"
################################################################################################

if [[ "${RHEL_RELEASE}" == "8" ]]; then
  "$(dirname "$0")/podman_main.sh"
  exit 0
fi
...
LOG "${DATE}: Docker config setup for client $DOCKER_SASFW\n"
LOG "Checking for required directories and configuration files:\n"

# create clients subdir based on DOCKER_SASFW variable dynamically added to vars on sasfw
MKDIR "$CLIENTS_DIR"
MKDIR "${CLIENTS_DIR}/${DOCKER_SASFW}"
MKDIR "${CLIENTS_DIR}/${DOCKER_SASFW}/logs"
MKDIR "${CLIENTS_DIR}/${DOCKER_SASFW}/Nginx"
MKDIR "${CLIENTS_DIR}/${DOCKER_SASFW}/certs"

# copy over keys
if [ ! -d "./keys" ];then
  LOGEXIT "keys not found in build file."
else
  LOG "copying keys directory to ${CLIENTS_DIR}/${DOCKER_SASFW} ..."
  cp -r ./keys ${CLIENTS_DIR}/${DOCKER_SASFW}
fi

# copy over logpusher cert/key files
LOG "copying logpusher cert/key files to  ${CLIENTS_DIR}/${DOCKER_SASFW}/certs/"
cp --force ./auth-logpush/client.* "${CLIENTS_DIR}/${DOCKER_SASFW}/certs"
[[ "$?" -ne 0 ]] && LOGEXIT "cp ./auth-logpush/client.* \"${CLIENTS_DIR}/${DOCKER_SASFW}/certs\" failed."
sha256sum "${CLIENTS_DIR}/${DOCKER_SASFW}/certs"/* 2>/dev/null
...
case "${TUNNEL_TYPE}" in 
  "ipsec")
    ...
    LOG "copying ipsec's docker-compose.yml to ${CLIENTS_DIR}/${DOCKER_SASFW}"
    cp ./docker-compose-ipsec.yml "${CLIENTS_DIR}/${DOCKER_SASFW}/docker-compose.yml"
    [[ "$?" -ne 0 ]] && LOGEXIT "cp ./docker-compose-ipsec.yml \"${CLIENTS_DIR}/${DOCKER_SASFW}/\" failed."
    SASGC_VERSION="$(cat /tmp/docker-container.sasgc-ipsec.tgz | docker load | grep "Loaded image" | cut -d":" -f3)"
    [[ "$?" -ne 0 ]] && LOGEXIT "Could not load sasgc container from /tmp/docker-container.sasgc-ipsec.tgz"
    cp ./keys/site0.secrets "${CLIENTS_DIR}/${DOCKER_SASFW}"
    [[ "$?" -ne 0 ]] && LOGEXIT "cp ./keys/site0.secrets \"${CLIENTS_DIR}/${DOCKER_SASFW}/\" failed."
    modprobe af_key
  ...
  *)
    ...
    LOG "copying docker-compose.yml to ${CLIENTS_DIR}/${DOCKER_SASFW}"
    cp ./docker-compose.yml "${CLIENTS_DIR}/${DOCKER_SASFW}"
    [[ "$?" -ne 0 ]] && LOGEXIT "cp ./docker-compose.yml \"${CLIENTS_DIR}/${DOCKER_SASFW}/\" failed."
    # Import docker images from /tmp & capture versions
    SASGC_VERSION="$(cat /tmp/docker-container.sasgc.tgz | docker load | grep "Loaded image" | cut -d":" -f3)"
...
CHECKER_VERSION="$(cat /tmp/docker-container.checker.tgz | docker load | grep "Loaded image" | cut -d":" -f3)"
[[ "$?" -ne 0 ]] && LOGEXIT "Could not load checker container from /tmp/docker-container.checker.tgz"

# Copy over vars file
LOG "copying vars file to ${CLIENTS_DIR}/${DOCKER_SASFW}/vars"
cp ./vars ${CLIENTS_DIR}/${DOCKER_SASFW}/vars
[[ "$?" -ne 0 ]] && LOGEXIT "cp ./vars ${CLIENTS_DIR}/${DOCKER_SASFW}/vars failed."
...
echo -e "SASFW=${DOCKER_SASFW}" >> ${CLIENT_ENV}
...
# --- Adding ipvlan & host-only gateways to compose's .env file -------------------------------
  DOCKER_IPVLAN_NETWORK="ipvlan"
  DOCKER_HOST_ONLY_NETWORK="host-only"
  if ! docker network ls | grep -qw "${DOCKER_IPVLAN_NETWORK}"; then
    echo "Docker network ${DOCKER_IPVLAN_NETWORK} is not present. Aborting"
    exit 1
  fi
  if ! docker network ls | grep -qw "${DOCKER_HOST_ONLY_NETWORK}"; then
    echo "Docker network ${DOCKER_IPVLAN_NETWORK} is not present. Aborting"
    exit 1
  fi
  PYTHON_GW_EXTRACTOR=(
    "import json;"
    "import sys;"
    "print json.loads(sys.stdin.read())[0]['IPAM']['Config'][0]['Gateway']"
  )
  IPVLAN_GW="$(\
    docker network inspect "${DOCKER_IPVLAN_NETWORK}" \
    | python2 -c "${PYTHON_GW_EXTRACTOR[*]}" \
  )"
  LOG "Found ipvlan gateway IP: ${IPVLAN_GW}"
  sed --in-place "/^IPVLAN_GW=.*/d" "${CLIENT_ENV}"
  echo "IPVLAN_GW=${IPVLAN_GW}" >> "${CLIENT_ENV}"
  HOST_ONLY_GW="$(\
    docker network inspect "${DOCKER_HOST_ONLY_NETWORK}" \
    | python2 -c "${PYTHON_GW_EXTRACTOR[*]}" \
  )"
...
# --------------- HA vars --------------------------------------------------------
  ...
  HA_WATCHDOG_SCRIPT="watchdog.sh"
...
# --------------- Install GCM Infrastructure -------------------------------------
  ...
  if ! install_gcm_infrastructure "./gcm" "${DOCKER_SASFW}"; then
...
# --------------- Ensure HA watchdog infrastructure is actual --------------------
  ...
  for service_unit in \
      ...
      "${SYSTEMD_UNIT_SGHA_WATCHDOG}.service" \
...
systemctl enable "${SYSTEMD_UNIT_SGHA_WATCHDOG}.service"
systemctl enable "sgha-updater@${DOCKER_SASFW}.service" && systemctl start "sgha-updater@${DOCKER_SASFW}.service"
# --------------------------------------------------------------------------------

LOG "Docker config complete for client $DOCKER_SASFW\n"
LOG "--------------------------------------------------------------"
LOG ""
LOG "For resiliency (single node setup) run the following commands:"
LOG ""
LOG "     systemctl enable ${SYSTEMD_UNIT_SGHA_RESILIENCY}@${DOCKER_SASFW}.service"
LOG "     systemctl start ${SYSTEMD_UNIT_SGHA_RESILIENCY}@${DOCKER_SASFW}.service"
LOG "--------------------------------------------------------------"
LOG "For HA (multi nodes setup) run the following commands:"
LOG "Ensure resiliency is disabled:"
LOG "     systemctl stop ${SYSTEMD_UNIT_SGHA_RESILIENCY}@${DOCKER_SASFW}.service"
LOG "     systemctl disable ${SYSTEMD_UNIT_SGHA_RESILIENCY}@${DOCKER_SASFW}.service"
LOG ""
LOG "On master machine:"
LOG "     systemctl enable ${SYSTEMD_UNIT_SGHA_MASTER}@${DOCKER_SASFW}.service"
LOG "     systemctl start ${SYSTEMD_UNIT_SGHA_MASTER}@${DOCKER_SASFW}.service"
LOG
LOG "On slave machine:"
LOG "     systemctl enable ${SYSTEMD_UNIT_SGHA_SLAVE}@${DOCKER_SASFW}.service"
LOG "     systemctl start ${SYSTEMD_UNIT_SGHA_SLAVE}@${DOCKER_SASFW}.service"
LOG
exit 0
