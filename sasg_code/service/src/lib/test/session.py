...
class SSH:
    ...
    def session(self, address, username, pass_key, port=22):
        ...
        if not os.path.isfile(pass_key):
            ...
            self.client.connect(address, username=username, pkey=pass_key, look_for_keys=False)
            self.transport = paramiko.Transport((address, port))
            self.transport.connect(username=username, pkey=pass_key)
    ...
    def openShell(self):
        #self.shell = self.client.invoke_shell(height=20, width=20) # max height, width
        self.shell = self.client.invoke_shell(height=65535, width=65535) # max height, width

        #print(self.shell.get_pty())
    ...
    def sendShell(self, command):
        ...
        while True:
            ...
            if escape > 60: # 1 minutes: error.
                break # if this fails, the subsequent will not work.

            cnt += 1
            if cnt > 65535:
                print('push space to move forward...')
                self.shell.send(" \n") # space to get moving.
                #self.shell.send("\n") # space to get moving.
                time.sleep(1)
                cnt = 0
    ...
    # TODO: learn this and make this more interactive... by allowing users to interact directly.
    def process(self):
        #global connection
        while True:
            # Output data when available
    ...
    # TODO: change this to multiple commands so that we can keep the session for multiple commands? str to array?
...
# SHELL TESTING
#ssh = SSH()
#ssh.session('174.36.252.183', 'vyos', 'P9kDqSbl')
#ssh.openShell()
#ssh.sendShell('configure')
#ssh.sendShell('show system')

#ssh.sendShell('exit')
#ssh.sendShell('exit')

#ssh.sendShell('stty size')
#ssh.sendShell('show configuration')
#ssh.sendShell('ls /')
