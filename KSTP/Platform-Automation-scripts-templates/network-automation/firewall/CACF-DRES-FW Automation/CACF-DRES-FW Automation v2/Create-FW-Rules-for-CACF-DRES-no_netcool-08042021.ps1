  ...
  Write-Host "USAGE: Create-FW-Rules-for-CACF-DRES <region> <segmentNumber> <CACF Proxy VIP Position> <OCP number>"
  ...
  $OPC_VIP   = Get-nsxipset -Name ("HOST-SRES-LB-OCP-Cluster" + $ocp_no + "-VIP")
  
  #Get SG Group
  Write-Host -foregroundcolor "Green" "Looking for SGs..."
  $OCP       =  Get-NsxSecurityGroup -Name ("SG-SRES01-OCP-Cluster0" + $ocp_no)
  $SRES_Linux_JH = Get-NsxSecurityGroup -Name SG-SRES-Linux-Jump-Servers
  $SRES_Windows_JH = Get-NsxSecurityGroup -Name SG-SRES-WIN-Jump-Servers
  ...
  # Ansible SSH
  Get-NsxFirewallSection $DRESSectionName | New-NsxFirewallRule -Name "SRES OCP3 to CACF Proxy VIP SSH" -Action allow -Source $OCP -Destination $CACF_ProxyVIP -Service $SSH -AppliedTo $proxy1_vm,$proxy2_vm,$OCP,$sres_esg1,$dres_esg1 -Disabled -Position bottom -EnableLogging
  
  
  Get-NsxFirewallSection $DRESSectionName | New-NsxFirewallRule -Name "CAFCF Proxy to CPREM JH SSH" -Action allow -Source $proxy1_vm,$proxy2_vm -Service $SSH -AppliedTo $proxy1_vm,$proxy2_vm,$dres_esg1,$dres_vpn1 -Disabled -Position bottom -EnableLogging
  ...
  # Backend Flow
  Get-NsxFirewallSection $DRESSectionName | New-NsxFirewallRule -Name "Inter CACF Proxy" -Source $proxy1_vm,$proxy2_vm -Destination $proxy1_vm,$proxy2_vm -Action allow -AppliedTo $proxy1_vm,$proxy2_vm  -Disabled -Position bottom
  
  #SRES Jump Hosts to Proxies
  Get-NsxFirewallSection $DRESSectionName | New-NsxFirewallRule -Name "CPREM JH to CAFCF Proxy BDS" -Action allow -Source $SRES_Linux_JH,$SRES_Windows_JH -Destination $proxy1_vm,$proxy2_vm,$CACF_ProxyVIP -Service $BDS -AppliedTo $proxy1_vm,$proxy2_vm,$dres_esg1,$sres_esg1,$SRES_Linux_JH,$SRES_Windows_JH -Disabled -Position bottom -EnableLogging
  
## Add NAT 
Write-Host -foregroundcolor "Green" "Creating NAT rules..."

./Create-NAT-for-CACF-DRES-08042021.ps1 $region $segmentNumber $vipip 
