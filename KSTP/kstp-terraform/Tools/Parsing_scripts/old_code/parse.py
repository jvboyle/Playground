...
myMenu = foldermenu.FolderBasedMenu(my_questions)
...
# Required paths
terraform_path      = '{}/terraform'.format(cwd)
export_files_path   = '{}/{}'.format(cwd, export_path)
init_config_path    = '{}/json_config/initial_config'.format(cwd)
tf_config_path      = '{}/json_config/terraform_config'.format(cwd)
tf_code_path        = '{}/tf_code'.format(tf_config_path)
...
if (myFiles.file_error):
    ...
    myLogger.info('   |-- The execution will be interrupted')
    myLoggerObj.log_terminate()
myFiles.create_folder('{}'.format(tf_code_path))
if (myFiles.file_error):
    myLogger.critical('   |-- Error message: {}'.format(myFiles.file_data))    
    myLogger.info('   |-- Please correct the error above and try again')
    myLogger.info('   |-- The execution will be interrupted')
...
try: 
    ...
    if (myFiles.file_exists(existing_rules_file)):
        ...
        if (result['error']):
            ...
            existing_rules = result['data']['existing_rules']
            if (existing_rules):
                # ATTENTION
...
try:
    ...
    if (new_services):
        ...
        if (listed_services):
            ...
            map_svc = dict(map(lambda x: (parse.fix_name(x), all_services[x]), listed_services))
        ...
        for service in not_listed_services:
            if (len(service) > 3):
                ...
                if (proto == 'tcp-') or (proto == 'udp-'):
                    ...
                    service = parse.fix_name(service)
                    map_svc.update({
                        service: {
                            service[:3]: ports
                        }
        ...
        result = myFiles.write_json_file('{}/services.json'.format(tf_config_path), services_terraform)
        if (result["error"]):
                myLogger.critical('      |-- Error creating the services file')
                myLogger.warning('      |-- Error message: {}'.format(result['data']))
                myLogger.info('Execution will be interrupted.')
                myLoggerObj.log_terminate()
...
try:
    ...
    myFiles.file_set_projects_path = projects_path
        ...
        if (dfw_section_name.startswith('TF-DRES') or dfw_section_name.startswith('TF-DRIAMAAS')):
                ...
                for key in json_section['map_policies'].keys():
                    for rule in json_section['map_policies'][key]["rules"]:
                        if (rule in not_migrated_rules):    # Came from the existing rules section above
                            ...
                            updated_srv_list = list(map(lambda x: parse.fix_name(x) if (x in existing_services_dict['should_migrate']) else x, json_section['map_policies'][key]['rules'][rule]['services']))
            ...
            for srv_name in tmp_json:
                if (srv_name in existing_services_dict['should_migrate']):
                    ...
                    json_files_content['DFW'][dfw_section_name]['map_services']['SERVICES'].append(parse.fix_name(srv_name))
            ...
            for srv_name in json_section['map_services']['SERVICES']:
                if (srv_name in existing_services_dict['should_migrate']):
                    updated_srv_list.append(parse.fix_name(srv_name))
                else:
            ...
            result = myFiles.write_json_file('{}/{}-{}.json'.format(tf_config_path, cs_name, dfw_section_name), json_section)
            if (result["error"]):
                myLogger.critical('      |-- Error creating Terraform dfw file')
                myLogger.warning('      |-- Error message: {}'.format(result['data']))
                myLogger.info('Execution will be interrupted.')
                myLoggerObj.log_terminate()
    ...
    if (dfw_dres_services):         
        ...
        result = myFiles.write_json_file('{}/{}_DRES.json'.format(tf_config_path, cs_name), dfw_data)
        if (result["error"]):
            myLogger.critical('      |-- Error creating DRES DFW file')
            myLogger.warning('      |-- Error message: {}'.format(result['data']))
            myLogger.info('Execution will be interrupted.')
            myLoggerObj.log_terminate()
        # Terraform format
...
try:
    ...
    if (result['error']):
        ...
        for key in to_be_migrated:
            if (not key in existing_groups):
                ...
                tf_code += dyn_footer
    myFiles.write_regular_file('{}/101-vm_groups.tf'.format(tf_code_path), tf_code)
    if (myFiles.file_error):
    ...
    if (result['error']):
        ...
        for key in to_be_migrated:
            ...
            if (len(ip_list) > 500):    # NSX-T limit: The total of IPAdressExpressions, MACAddressExpressions, paths in a PathExpression and external IDs in ExternalIDExpression should not exceed 500
                ...
                tf_code += dyn_ipexpression.format(ip_list)
                tf_code += dyn_footer
        myFiles.write_regular_file('{}/102-ipset_groups.tf'.format(tf_code_path), tf_code)
        if (myFiles.file_error):
    ...
    tf_code = """#####################################\n# EMPTY GROUPS\n#\n# Migration date: {}\n# Geography.....: {} \n# Wave..........: {}\n#####################################\n""".format(migration_date, geography, cs_name)
    empty_groups = json_files_content['SG']['static_groups']['empty_groups']
    to_be_migrated = list(set(empty_groups) - set(existing_groups))
    for key in empty_groups:
        result = parse.tf_prefix(geography, key, 'SG')
        if (result['error']):
        ...
        tf_code += dyn_header.format(grp_index, key, key)
        tf_code += dyn_footer
    myFiles.write_regular_file('{}/103-empty_groups.tf'.format(tf_code_path), tf_code)
    if (myFiles.file_error):
        ...
        for member_type in all_sgem_groups[key]['members'].keys():
            ...
            if (member_type == 'IPSet') or (member_type == 'SecurityGroup'):
                ...
                for member in all_sgem_groups[key]['members'][member_type]:
                    result = parse.tf_prefix(geography, member, m_type)
                    if (result['error']):
    ...
    myFiles.write_regular_file('{}/104-static_groups.tf'.format(tf_code_path), tf_code)
    if (myFiles.file_error):
        myLogger.critical('         |-- Error saving empty groups file: {}'.format(result['data']))        
        myLogger.info('         |-- The execution will be interrupted')
        myLoggerObj.log_terminate()
    ...
    myFiles.write_regular_file('{}/105-dynamic_groups.tf'.format(tf_code_path), tf_code)
    if (myFiles.file_error):
        myLogger.critical('         |-- Error saving empty groups file: {}'.format(result['data']))        
        myLogger.info('         |-- The execution will be interrupted')
        myLoggerObj.log_terminate()
...
if (result['error']):
    myLogger.critical('      |-- Error saving {} file'.format(save_to))
    myLogger.warning('      |-- Error message: {}'.format(result['data']))
    myLogger.info('Execution will be interrupted.')
    myLoggerObj.log_terminate()
else:
...
try:
    ...
    for grp in all_groups:
        new_grp = parse.tf_prefix(geography, grp, 'SG')
