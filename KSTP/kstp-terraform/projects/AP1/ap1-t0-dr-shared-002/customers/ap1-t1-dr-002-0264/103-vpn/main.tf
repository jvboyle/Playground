...
resource "nsxt_policy_ipsec_vpn_service" "vpn_264" {
  display_name        = "${var.customer}-vpn1"
  description         = "${var.customer} dres264 service"
  locale_service_path = data.nsxt_policy_gateway_locale_service.select.path # /infra/tier-1s/ap1-t1-dr-002-0264
  enabled             = true
  ha_sync             = true
  ike_log_level       = "INFO"
}
...
resource "nsxt_policy_ipsec_vpn_local_endpoint" "local_ep" {
  ...
  service_path  = nsxt_policy_ipsec_vpn_service.vpn_264.path
  description   = "${var.customer} dres264 vpn1"
  local_address = "168.1.103.240"
}

resource "nsxt_policy_ipsec_vpn_session" "EA-session1" {
  display_name               = "${var.customer}-EA-session1"
  ...
  peer_address               = "203.14.81.253"
  peer_id                    = "203.14.81.253"
  psk                        = "CHANGE_ME"
  connection_initiation_mode = "INITIATOR"
  rule {
    sources      = ["158.87.104.0/24"]
    destinations = ["10.240.240.16/32","10.240.142.241/32","10.172.203.0/28","10.226.11.0/24","10.240.132.21/32","10.240.132.22/32"]
    action       = "PROTECT"
  }
}
