...
resource "nsxt_policy_ipsec_vpn_service" "vpn_232" {
  display_name        = "${var.customer}-vpn1"
  description         = "${var.customer} dres232 service"
  locale_service_path = data.nsxt_policy_gateway_locale_service.select.path # /infra/tier-1s/ap1-t1-dr-002-0232
  enabled             = true
  ha_sync             = true
  ike_log_level       = "INFO"
}
...
resource "nsxt_policy_ipsec_vpn_local_endpoint" "local_ep" {
  ...
  service_path  = nsxt_policy_ipsec_vpn_service.vpn_232.path
  description   = "${var.customer} dres232 vpn1"
  local_address = "168.1.103.233"
}

resource "nsxt_policy_ipsec_vpn_session" "session1" {
  display_name               = "${var.customer}-WNZ-session"
  ...
  peer_address               = "202.7.37.4"
  peer_id                    = "202.7.37.4"
  psk                        = "CHANGE_ME"
  connection_initiation_mode = "INITIATOR"
  rule {
    sources      = ["158.87.104.0/24"]
    destinations = ["202.7.36.176/29"]
    action       = "PROTECT"
  }
}
...
resource "nsxt_policy_ipsec_vpn_session" "session2" {
  display_name               = "${var.customer}-MCI-session"
  description                = "Terraform-provisioned IPsec Policy-Based VPN"
  ike_profile_path           = "/infra/ipsec-vpn-ike-profiles/IKEv1_aes256_sha1_14_28800"
  tunnel_profile_path        = "/infra/ipsec-vpn-tunnel-profiles/esp_aes256_sha1_14"
  local_endpoint_path        = nsxt_policy_ipsec_vpn_local_endpoint.local_ep.path
  dpd_profile_path           = "/infra/ipsec-vpn-dpd-profiles/nsx-default-l3vpn-dpd-profile"
  enabled                    = true
  service_path               = nsxt_policy_ipsec_vpn_service.vpn_232.path
