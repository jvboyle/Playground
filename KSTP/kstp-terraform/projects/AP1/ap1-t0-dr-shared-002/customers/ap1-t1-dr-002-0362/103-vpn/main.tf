...
resource "nsxt_policy_ipsec_vpn_service" "vpn_362" {
  display_name        = "${var.customer}-vpn1"
  description         = "${var.customer} dres362 service"
  locale_service_path = data.nsxt_policy_gateway_locale_service.select.path # /infra/tier-1s/ap1-t1-dr-002-0362
  enabled             = true
  ha_sync             = true
  ike_log_level       = "INFO"
}
...
resource "nsxt_policy_ipsec_vpn_local_endpoint" "local_ep" {
  ...
  service_path  = nsxt_policy_ipsec_vpn_service.vpn_362.path
  description   = "${var.customer} dres362 vpn1"
  local_address = "168.1.103.210"
}

resource "nsxt_policy_ipsec_vpn_session" "session1" {
  display_name               = "${var.customer}-CPX-session"
  ...
  peer_address               = "114.7.227.22"
  peer_id                    = "114.7.227.22"
  psk                        = "CHANGE_ME"
  connection_initiation_mode = "INITIATOR"
  rule {
    sources      = ["158.87.104.0/24"]
    destinations = ["10.20.0.0/16","10.10.0.0/16"]
    action       = "PROTECT"
  }
}
