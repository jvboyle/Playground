...
resource "nsxt_policy_ipsec_vpn_service" "vpn_141" {
  display_name        = "${var.customer}-vpn1"
  description         = "${var.customer} dres141 service"
  locale_service_path = data.nsxt_policy_gateway_locale_service.select.path # /infra/tier-1s/ap1-t1-dr-002-0141
  enabled             = true
  ha_sync             = true
  ike_log_level       = "INFO"
}
...
resource "nsxt_policy_ipsec_vpn_local_endpoint" "local_ep" {
  ...
  service_path  = nsxt_policy_ipsec_vpn_service.vpn_141.path
  description   = "${var.customer} dres141 vpn1"
  local_address = "168.1.103.231"
}
...
resource "nsxt_policy_ipsec_vpn_session" "session1" {
  ...
  peer_address               = "161.202.179.194"
  peer_id                    = "161.202.179.194"
  psk                        = "CHANGE_ME"
  connection_initiation_mode = "INITIATOR"
  rule {
    sources      = ["158.87.104.0/24"]
    destinations = ["10.116.103.175/32"]
    action       = "PROTECT"
  }
}
