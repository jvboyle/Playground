...
resource "nsxt_policy_ipsec_vpn_service" "vpn_256" {
  display_name        = "${var.customer}-vpn1"
  description         = "${var.customer} dres256 service"
  locale_service_path = data.nsxt_policy_gateway_locale_service.select.path # /infra/tier-1s/ap1-t1-dr-001-0256
  enabled             = true
  ha_sync             = true
  ike_log_level       = "INFO"
}
...
resource "nsxt_policy_ipsec_vpn_local_endpoint" "local_ep" {
  ...
  service_path  = nsxt_policy_ipsec_vpn_service.vpn_256.path
  description   = "${var.customer} dres256 vpn1"
  local_address = "168.1.103.237"
}
...
resource "nsxt_policy_ipsec_vpn_session" "session1" {
  ...
  peer_address               = "20.157.95.22"
  peer_id                    = "20.157.95.22"
  psk                        = "CHANGE_ME"
  connection_initiation_mode = "INITIATOR"
  rule {
    sources      = ["158.87.104.0/24"]
    destinations = ["172.16.4.0/24"]
    action       = "PROTECT"
  }
}
