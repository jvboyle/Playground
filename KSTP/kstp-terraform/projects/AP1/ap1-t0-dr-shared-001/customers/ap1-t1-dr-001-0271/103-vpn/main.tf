...
resource "nsxt_policy_ipsec_vpn_service" "vpn" {
  display_name        = "${var.customer}-vpn"
  description         = "${var.customer}-vpn"
...
resource "nsxt_policy_ipsec_vpn_session" "session1" {
  ...
  peer_address               = "103.129.154.242"
  peer_id                    = "103.129.154.242"
  psk                        = "CHANGE_ME"
  connection_initiation_mode = "INITIATOR"
  rule {
    sources      = ["158.87.104.0/24"]
    destinations = ["10.72.8.0/24"]
    action       = "PROTECT"
  }
}
