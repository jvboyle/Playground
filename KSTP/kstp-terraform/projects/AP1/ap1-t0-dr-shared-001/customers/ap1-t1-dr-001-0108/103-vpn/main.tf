...
resource "nsxt_policy_ipsec_vpn_service" "vpn_108" {
  display_name        = "${var.customer}-vpn1"
  description         = "${var.customer} dres108 service"
  locale_service_path = data.nsxt_policy_gateway_locale_service.select.path # /infra/tier-1s/ap1-t1-dr-001-0108
  enabled             = true
  ha_sync             = true
  ike_log_level       = "INFO"
}
...
resource "nsxt_policy_ipsec_vpn_local_endpoint" "local_ep" {
  ...
  service_path  = nsxt_policy_ipsec_vpn_service.vpn_108.path
  description   = "${var.customer} dres108 vpn1"
  local_address = "168.1.103.225"
}
...
resource "nsxt_policy_ipsec_vpn_session" "session1" {
  ...
  peer_address               = "110.165.25.132"
  peer_id                    = "110.165.25.132"
  psk                        = "CHANGE_ME"
  connection_initiation_mode = "INITIATOR"
  rule {
    sources      = ["158.87.104.0/24"]
    destinations = ["24.100.100.3/32"]
    action       = "PROTECT"
  }
}
