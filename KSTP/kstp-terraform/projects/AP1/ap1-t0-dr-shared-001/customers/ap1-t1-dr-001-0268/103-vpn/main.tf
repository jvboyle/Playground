...
resource "nsxt_policy_ipsec_vpn_service" "vpn_268" {
  display_name        = "${var.customer}-vpn1"
  description         = "${var.customer} dres268 service"
  locale_service_path = data.nsxt_policy_gateway_locale_service.select.path # /infra/tier-1s/ap1-t1-dr-001-0268
  enabled             = true
  ha_sync             = true
  ike_log_level       = "INFO"
}
...
resource "nsxt_policy_ipsec_vpn_local_endpoint" "local_ep" {
  ...
  service_path  = nsxt_policy_ipsec_vpn_service.vpn_268.path
  description   = "${var.customer} dres268 vpn1"
  local_address = "168.1.103.241"
}
...
resource "nsxt_policy_ipsec_vpn_session" "session1" {
  ...
  peer_address               = "203.35.0.38"
  peer_id                    = "203.35.0.38"
  psk                        = "CHANGE_ME"
  connection_initiation_mode = "INITIATOR"
  rule {
    sources      = ["158.87.104.0/24"]
    destinations = ["10.95.30.136/30"]
    action       = "PROTECT"
  }
}
