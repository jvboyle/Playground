...
resource "nsxt_policy_nat_rule" "dnat1" {
  ...
  action               = "DNAT"
  source_networks      = ["10.95.30.136/30"]
  destination_networks = ["158.87.104.50/32"]
  translated_networks  = ["100.80.126.50/32"]
...
resource "nsxt_policy_nat_rule" "dnat2" {
  ...
  translated_networks  = ["100.80.126.51/32"]
  gateway_path         = data.nsxt_policy_tier1_gateway.select.path
  logging              = true
  firewall_match       = "MATCH_INTERNAL_ADDRESS"
...
resource "nsxt_policy_nat_rule" "dnat3" {
  ...
  translated_networks  = ["100.80.126.52/32"]
  gateway_path         = data.nsxt_policy_tier1_gateway.select.path
  logging              = true
  firewall_match       = "MATCH_INTERNAL_ADDRESS"
...
resource "nsxt_policy_nat_rule" "snat1" {
  ...
  action               = "SNAT"
  source_networks      = ["100.80.126.51/32"] #Ap1 Subnet 
  destination_networks = ["10.95.30.136/30"]
...
resource "nsxt_policy_nat_rule" "snat2" {
  ...
  action               = "SNAT"
  source_networks      = ["100.80.126.52/32"] #Ap1 Subnet 
  destination_networks = ["10.95.30.136/30"]
