...
resource "nsxt_policy_ipsec_vpn_service" "vpn_334" {
  display_name        = "${var.customer}-vpn1"
  description         = "${var.customer} dres334 service"
  locale_service_path = data.nsxt_policy_gateway_locale_service.select.path # /infra/tier-1s/ag1-t1-dr-001-0334
  enabled             = true
  ha_sync             = true
  ike_log_level       = "INFO"
}
...
resource "nsxt_policy_ipsec_vpn_local_endpoint" "local_ep" {
  ...
  service_path  = nsxt_policy_ipsec_vpn_service.vpn_334.path
  description   = "${var.customer} dres334 vpn1"
  local_address = "150.239.66.218"
}
...
resource "nsxt_policy_ipsec_vpn_session" "session1" {
  ...
  peer_address               = "184.81.115.12"
  peer_id                    = "184.81.115.12"
  psk                        = "gFRKBitrOPBKByWQxeCpW2gt"
  connection_initiation_mode = "INITIATOR"
  rule {
    sources      = ["158.87.104.0/24"]
    destinations = ["216.96.68.0/22",
                    "71.29.160.0/21"
                   ]
    action       = "PROTECT"
  }
