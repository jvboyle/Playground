...
resource "nsxt_policy_group" "grp_4" {
   display_name = "TF-${var.environment}-IPSET-DR${local.dres}-${local.consumable_service}-CPREM-NETCOOL"
   nsx_id = "TF-${var.environment}-IPSET-DR${local.dres}-${local.consumable_service}-CPREM-NETCOOL"
   description  = local.change
   tag {
      tag = "TF-${var.customer}"
   }
