...
resource "nsxt_policy_ipsec_vpn_local_endpoint" "local_ep" {
  ...
  local_address = "169.38.109.216"                       # update with Endpoint ip,
}
# 
module "vpn" {
  ...
  endpoint_local_address      = "169.38.109.216"
  ...
  peer_address                = "169.38.124.20" # Remote peer ip 
  next_hop                    = "169.38.109.193" # Juniper ip 

  bypass_rule_config = {
    sources      = ["158.87.104.0/24"]
    destinations = ["172.22.1.21/32", "172.22.1.22/32"]
  }
}
