...
resource "nsxt_policy_ipsec_vpn_local_endpoint" "local_ep" {
  ...
  local_address = "169.38.109.201"                       # update with Endpoint ip,
}
# 
module "vpn-1" {
  ...
  endpoint_local_address      = "169.38.109.201"
  ...
  peer_address                = "103.75.249.200" # Remote peer ip 
  next_hop                    = "169.38.109.193" # Juniper ip 

  bypass_rule_config = {
    sources      = ["158.87.104.0/24"]
    destinations = ["172.18.134.225/32", "172.18.134.226/32"]
  }
}
