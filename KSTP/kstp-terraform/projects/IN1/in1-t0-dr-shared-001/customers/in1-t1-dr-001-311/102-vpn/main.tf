...
resource "nsxt_policy_ipsec_vpn_local_endpoint" "local_ep" {
  ...
  local_address = "169.38.109.200"                       # update with Endpoint ip,
}
# 
module "vpn-1" {
  ...
  endpoint_local_address      = "169.38.109.200"
  ...
  peer_address                = "115.110.228.96" # Remote peer ip 
  next_hop                    = "169.38.109.193" # Juniper ip 

  bypass_rule_config = {
    sources      = ["158.87.104.0/24"]
    destinations = ["172.27.0.64/32", "172.27.0.65/32"]
  }
}
