...
resource "nsxt_policy_ipsec_vpn_service" "vpn_335" {
  display_name        = "${var.customer}-vpn1"
  description         = "${var.customer} dres335 service"
  locale_service_path = data.nsxt_policy_gateway_locale_service.select.path # /infra/tier-1s/eu1-t1-dr-001-335
  enabled             = true
  ha_sync             = true
  ike_log_level       = "INFO"
}
...
resource "nsxt_policy_ipsec_vpn_local_endpoint" "local_ep" {
  ...
  service_path  = nsxt_policy_ipsec_vpn_service.vpn_335.path
  description   = "${var.customer} dres335 vpn1"
  local_address = "169.51.163.29"
}

resource "nsxt_policy_ipsec_vpn_session" "UNICAJA_BANCO" {
  display_name               = "${var.customer}-UNICAJA_BANCO"
  description                = "RITM0152458"
  ...
  peer_address               = "195.76.110.196"
  peer_id                    = "195.76.110.196"
  psk                        = "CHANGE_ME"
  connection_initiation_mode = "INITIATOR"
  rule {
    sources      = ["158.87.104.0/24"]
    destinations = ["172.31.105.0/24", "172.31.140.128/27", "172.22.118.0/24"]
    action       = "PROTECT"
  }
}
