...
resource "nsxt_policy_nat_rule" "dnat1" {
  ...
  translated_networks  = ["100.74.2.50/32"]
...
resource "nsxt_policy_nat_rule" "snat1" {
  ...
  action               = "SNAT"
  source_networks      = ["100.74.2.51/32"] #Ap1 Subnet 
  destination_networks = ["172.28.26.0/28"]
...
resource "nsxt_policy_nat_rule" "snat2" {
  ...
  action               = "SNAT"
  source_networks      = ["100.74.2.52/32"] #Ap1 Subnet 
  destination_networks = ["172.28.26.0/28"]
