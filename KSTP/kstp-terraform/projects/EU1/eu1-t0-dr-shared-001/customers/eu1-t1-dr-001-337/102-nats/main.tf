...
resource "nsxt_policy_nat_rule" "dnat1" {
  ...
  translated_networks  = ["100.73.246.50/32"]
...
resource "nsxt_policy_nat_rule" "snat1" {
  ...
  action               = "SNAT"
  source_networks      = ["100.73.246.51/32"] #Ap1 Subnet 
  destination_networks = ["151.171.0.0/16"]
...
resource "nsxt_policy_nat_rule" "snat2" {
  ...
  action               = "SNAT"
  source_networks      = ["100.73.246.52/32"] #Ap1 Subnet 
  destination_networks = ["151.171.0.0/16"]
